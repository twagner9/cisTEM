...
void CTFRefinementManager::RunRefinementJob( ) {
    ...
    if ( my_parent->RefineCTFCheckBox->IsChecked( ) == true && my_parent->RefineBeamTiltCheckBox->IsChecked( ) == true ) {
        ...
        output_refinement->name = wxString::Format("Beam Tilt Refinement #%li", current_output_refinement_id);
    ...
    for ( int class_counter = 0; class_counter < active_refinement_package->number_of_classes; class_counter++ ) {
        ...
        if ( active_ctf_refinement == true )
            ...
            output_refinement->class_refinement_results[class_counter].high_resolution_limit = input_refinement->class_refinement_results[class_counter].high_resolution_limit;
    ...
    else if ( active_ctf_refinement == true ) {
...
void CTFRefinementManager::SetupReconstructionJob( ) {
    ...
    if ( number_of_particles - number_of_reconstruction_jobs < number_of_reconstruction_jobs )
        particles_per_job = 1;
    else
    ...
    for ( class_counter = 0; class_counter < active_refinement_package->number_of_classes; class_counter++ ) {
        ...
        for ( job_counter = 0; job_counter < number_of_reconstruction_jobs; job_counter++ ) {
            ...
            if ( current_particle_counter > number_of_particles || job_counter == number_of_reconstruction_jobs - 1 )
                current_particle_counter = number_of_particles;
...
void CTFRefinementManager::SetupRefinementJob( ) {
    ...
    number_of_particles = active_refinement_package->contained_particles.GetCount( );
    if ( number_of_particles - number_of_refinement_jobs < number_of_refinement_jobs )
        particles_per_job = 1;
    else
    ...
    for ( counter = 0; counter < number_of_refinement_jobs; counter++ ) {
        ...
        current_particle_counter += particles_per_job;
        if ( current_particle_counter > number_of_particles || counter == number_of_refinement_jobs )
            current_particle_counter = number_of_particles;
...
void CTFRefinementManager::ProcessJobResult(JobResult* result_to_process) {
    ...
    else if ( running_job_type == ESTIMATE_BEAMTILT ) {
...
void CTFRefinementManager::DoMasking( ) {
    ...
    if ( active_should_mask == true ) // user selected masking
    {
        ...
        auto stop_flag            = std::make_shared<std::atomic<bool>>(false);
        my_parent->masking_thread = new AutoMaskerThread(my_parent, current_reference_filenames, masked_filenames, input_refinement->resolution_statistics_pixel_size, active_refinement_package->estimated_particle_size_in_angstroms * 0.75, stop_flag, false, my_parent->active_mask_thread_id);
