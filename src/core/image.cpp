...
void Image::ExtractSlice(Image& image_to_extract, AnglesAndShifts& angles_and_shifts_of_image, float resolution_limit, bool apply_resolution_limit) {
    ...
    if ( image_to_extract.logical_x_dimension != logical_x_dimension || image_to_extract.logical_y_dimension != logical_y_dimension )
        padding = true;
    ...
    if ( padding ) {
        ...
        if ( temp_image->logical_x_dimension < image_to_extract.logical_x_dimension && temp_image->logical_y_dimension < image_to_extract.logical_y_dimension )
...
void Image::AddByLinearInterpolationFourier2D(float& wanted_logical_x_coordinate, float& wanted_logical_y_coordinate, std::complex<float>& wanted_value) {
    ...
    for ( j = int_y_coordinate; j <= int_y_coordinate + 1; j++ ) {
        ...
        for ( i = int_x_coordinate; i <= int_x_coordinate + 1; i++ ) {
            if ( i >= logical_lower_bound_complex_x && i <= logical_upper_bound_complex_x && j >= logical_lower_bound_complex_y && j <= logical_upper_bound_complex_y ) {
    ...
    for ( j = 0; j <= physical_upper_bound_complex_y; j++ ) {
        ...
        for ( i = 0; i <= physical_upper_bound_complex_x; i++ ) {
            ...
            if ( calculate_complex_ctf ) {
                ...
                if ( apply_coherence_envelope )
                    complex_values[pixel_counter] = ctf_of_image.EvaluateWithEnvelope(frequency_squared, azimuth) + I * 0.0f;
                else
        ...
        for ( k = 0; k < logical_z_dimension; k++ ) {
            ...
            for ( j = 0; j < logical_y_dimension; j++ ) {
                ...
                for ( i = 0; i < logical_x_dimension; i++ ) {
                    ...
                    else if ( distance_from_center_squared <= inner_mask_radius_squared && distance_from_center_squared >= inner_mask_radius_minus_edge_squared && wanted_inner_radius > 0.0 ) {
                        distance_from_center       = sqrtf(distance_from_center_squared);
                    ...
                    else if ( distance_from_center_squared <= inner_mask_radius_minus_edge_squared && wanted_inner_radius > 0.0 ) {
            ...
            for ( j = 0; j < logical_y_dimension; j++ ) {
                ...
                for ( i = 0; i < logical_x_dimension; i++ ) {
                    ...
                    if ( ! (x <= mask_radius_x && y <= mask_radius_y && z <= mask_radius_z) && (x < mask_radius_plus_edge_x && y < mask_radius_plus_edge_y && z < mask_radius_plus_edge_z) ) {
                        ...
                        if ( x >= mask_radius_plus_edge_x || y >= mask_radius_plus_edge_y || z >= mask_radius_plus_edge_z )
                            real_values[pixel_counter] = pixel_sum;
                        else {
                            mask_volume += 1.0f;
                        }
        ...
        for ( k = 0; k < logical_z_dimension; k++ ) {
            ...
            for ( j = 0; j < logical_y_dimension; j++ ) {
                ...
                for ( i = 0; i < logical_x_dimension; i++ ) {
                    ...
                    else if ( x >= mask_radius_plus_edge_x || y >= mask_radius_plus_edge_y || z >= mask_radius_plus_edge_z )
                        real_values[pixel_counter] = pixel_sum;
                    else {
                        mask_volume += 1.0f;
                    }
...
void Image::ApplyLocalResolutionFilter(Image& local_resolution_map, float pixel_size, int wanted_number_of_levels) {
    ...
    while ( current_filter_freq >= pixel_size / max_res_Angstroms - filter_freq_step_size - 0.01 ) {
        ...
        if ( small_step_size ) {
            ...
            if ( filter_counter > 1 )
                current_filter.ApplyCosineMask(previous_filter_freq - 0.75 * cosine_falloff_width, cosine_falloff_width, true);
...
void Image::ComputeFilteredAmplitudeSpectrumFull2D(Image* average_spectrum_masked, Image* current_power_spectrum, float& average, float& sigma, float minimum_resolution, float maximum_resolution, float pixel_size_for_fitting, bool apply_cosine_mask) {
    ...
    this->ComputeAverageAndSigmaOfValuesInSpectrum(float(this->logical_x_dimension) * pixel_size_for_fitting / minimum_resolution, float(this->logical_x_dimension), average, sigma, 12);
    ...
    convolution_box_size = int(float(this->logical_x_dimension) * pixel_size_for_fitting / minimum_resolution * sqrt(2.0));
    ...
    this->SpectrumBoxConvolution(current_power_spectrum, convolution_box_size, float(this->logical_x_dimension) * pixel_size_for_fitting / minimum_resolution);
            ...
            for ( ii = 0; ii < other_image->logical_x_dimension; ii++ ) {
                ...
                if ( k < 0 || k >= logical_z_dimension || j < 0 || j >= logical_y_dimension || i < 0 || i >= logical_x_dimension ) {
                    ...
                    if ( other_image->real_values[pixel_counter] > threshold_value )
                        real_values[ReturnReal1DAddressFromPhysicalCoord(i, j, k)] += other_image->real_values[pixel_counter];
                }
            ...
            for ( jj = 0; jj <= other_image->physical_upper_bound_complex_y; jj++ ) {
                ...
                for ( ii = 0; ii <= other_image->physical_upper_bound_complex_x; ii++ ) {
                    ...
                    if ( fill_with_noise == false )
                        other_image->complex_values[pixel_counter] = ReturnComplexPixelFromLogicalCoord(temp_logical_x, temp_logical_y, temp_logical_z, wanted_padding_value + I * 0.0f);
                    else {
...
void Image::GetRealValueByLinearInterpolationNoBoundsCheckImage(float& x, float& y, float& interpolated_value) {
    ...
    interpolated_value = x_dist_m * y_dist_m * real_values[address_1] + x_dist * y_dist_m * real_values[address_1 + 1] + x_dist_m * y_dist * real_values[address_2] + x_dist * y_dist * real_values[address_2 + 1];
}
...
void Image::Resize(int wanted_x_dimension, int wanted_y_dimension, int wanted_z_dimension, float wanted_padding_value) {
    ...
    if ( logical_x_dimension == wanted_x_dimension && logical_y_dimension == wanted_y_dimension && logical_z_dimension == wanted_z_dimension )
        return;
...
void Image::RealSpaceBinning(int bin_x, int bin_y, int bin_z, bool symmetrical, bool exclude_incomplete_bins) {
    ...
    MyDebugAssertTrue(! (symmetrical && exclude_incomplete_bins), "symmetrical and exclude_incomplete_bins flags cannot be active at the same time");
...
void Image::SharpenMap(float pixel_size, float resolution_limit, bool invert_hand, float inner_mask_radius, float outer_mask_radius, float start_res_for_whitening, float additional_bfactor_low, float additional_bfactor_high, float filter_edge, bool should_auto_mask, Image* input_mask, ResolutionStatistics* input_resolution_statistics, float statistics_scale_factor, Curve* original_log_plot, Curve* sharpened_log_plot) {
    ...
    if ( sharpened_log_plot != NULL )
        sharpened_log_plot->SetupXAxis(0.0, 0.5 * sqrtf(3.0), int((logical_x_dimension / 2.0 + 1.0) * sqrtf(3.0) + 1.0));
        ...
        for ( j = 0; j <= physical_upper_bound_complex_y; j++ ) {
            ...
            for ( i = 0; i <= physical_upper_bound_complex_x; i++ ) {
                ...
                if ( radial_derivative )
                    complex_values[pixel_counter] *= 2.0f * PI * (x_coord * x_coord + y_coord * y_coord + z_coord * z_coord);
                else
                    complex_values[pixel_counter] *= 2.0f * (float)PI * I * (x_coord * unit_x + y_coord * unit_y + z_coord * unit_z);
                pixel_counter++;
            }
        }
    }
...
float Image::FindBeamTilt(CTF& input_ctf, float pixel_size, Image& phase_error_output, Image& beamtilt_output, Image& difference_image, float& beamtilt_x, float& beamtilt_y, float& particle_shift_x, float& particle_shift_y, float phase_multiplier, bool progress_bar, int first_position_to_search, int last_position_to_search, MyApp* app_for_result) {
    ...
    for ( current_beamtilt_azimuth = beam_tilt_azimuth_search_start_value; current_beamtilt_azimuth <= beam_tilt_azimuth_search_end_value; current_beamtilt_azimuth += beam_tilt_azimuth_search_step_size ) {
        for ( current_particle_shift_azimuth = particle_shift_azimuth_search_start_value; current_particle_shift_azimuth <= particle_shift_azimuth_search_end_value; current_particle_shift_azimuth += particle_shift_azimuth_search_step_size ) {
            for ( current_beamtilt = beam_tilt_search_start_value; current_beamtilt <= beam_tilt_search_end_value; current_beamtilt += beam_tilt_search_step_size ) {
    ...
    if ( test_significance > MINIMUM_BEAM_TILT_SIGNIFICANCE_SCORE / 10.0f ) // 10x less stringent than our current final condition. FIXME these should be specified somehwere else
    {
        ...
        simplex_minimzer.initial_values[1][2] = (best_beamtilt_azimuth + best_particle_shift_azimuth) * simplex_minimzer.value_scalers[2];
...
*/
    if ( ReturnCorrelationBetweenTwoHorizontalLines(1, 2) > correlation_threshold )
        weird_edge = true;
    else if ( ReturnCorrelationBetweenTwoHorizontalLines(logical_y_dimension - 2, logical_y_dimension - 3) > correlation_threshold )
        weird_edge = true;
    else if ( ReturnCorrelationBetweenTwoVerticalLines(1, 2) > correlation_threshold )
        weird_edge = true;
    else if ( ReturnCorrelationBetweenTwoVerticalLines(logical_x_dimension - 2, logical_x_dimension - 3) > correlation_threshold )
        weird_edge = true;
...
void Image::HelicalAverage(bool reverse_hand, int start_z, int end_z, float min_radius, float max_radius, float axial_translation, float azimuthal_rotation, float& cross_corr) {
    MyDebugAssertTrue(is_in_memory, "Memory not allocated");
    MyDebugAssertTrue(is_in_real_space, "Not in real space");
    MyDebugAssertFalse(logical_z_dimension == 1, "Must be 3D");
    long  x_current;
    long  y_current;
    long  z_current;
    float x_distance;
    ...
    Image copied_vol;
    copied_vol.CopyFrom(this);
    this->SetToConstant(0.0);
    if ( reverse_hand )
        azimuthal_rotation *= -1.0;

    copied_vol.Normalize(1.0f, max_radius);
    for ( z_current = start_z; z_current <= end_z; z_current++ ) {
        for ( y_current = 0; y_current < logical_y_dimension; y_current++ ) {
            for ( x_current = 0; x_current < logical_x_dimension; x_current++ ) { // Get index of coordinate
                x_distance = static_cast<float>(x_current) - physical_address_of_box_center_x;
                y_distance = static_cast<float>(y_current) - physical_address_of_box_center_y + 1;

                radius = sqrt(x_distance * x_distance + y_distance * y_distance);

                if ( radius > max_radius || radius < min_radius )
                    continue;

                current_index = copied_vol.ReturnReal1DAddressFromPhysicalCoord(x_current, y_current, z_current);
                z_translated  = (float)z_current;
                current_theta = 0;
                num_points    = 1;
                sum_dens      = copied_vol.real_values[current_index];

                // First move forward in z
                z_translated += axial_translation;
                current_theta += azimuthal_rotation;
                while ( z_translated <= (float)end_z ) {
                    rotated_x = static_cast<float>(physical_address_of_box_center_x) + (cos(current_theta) * x_distance) - (sin(current_theta) * y_distance);
                    rotated_y = static_cast<float>(physical_address_of_box_center_y) + 1 + (sin(current_theta) * x_distance) + (cos(current_theta) * y_distance);
                    sum_dens += copied_vol.TrilinearInterpolation(rotated_x, rotated_y, z_translated);
                    num_points++;
                    z_translated += axial_translation;
                    current_theta += azimuthal_rotation;
                }
                ...
                z_translated  = (float)z_current - axial_translation;
                current_theta = -1 * azimuthal_rotation;
                while ( z_translated >= (float)start_z ) {
                    rotated_x = static_cast<float>(physical_address_of_box_center_x) + (cos(current_theta) * x_distance - sin(current_theta) * y_distance);
                    rotated_y = static_cast<float>(physical_address_of_box_center_y) + 1 + (sin(current_theta) * x_distance + cos(current_theta) * y_distance);
                    sum_dens += copied_vol.TrilinearInterpolation(rotated_x, rotated_y, z_translated);
                    num_points++;
                ...
                this->real_values[current_index] = average;
            }
        }
    }

    // Normalize the averaged volume and calculate the cross correlation
    // between the original and the averaged volume
    Normalize(1.0f, max_radius);

    cross_corr = ReturnCorrelationCoefficientUnnormalized(copied_vol, max_radius);
}
                ...
                for ( i = 0; i < int_edge; i++ ) {
                    if ( i + j + k > 0 ) {
                        ...
                        if ( radius_squared <= edge_squared ) {
                            ...
                            if ( i > 0 && j > 0 ) {
                                pixel_counter                           = ReturnReal1DAddressFromPhysicalCoord(logical_x_dimension - i, logical_y_dimension - j, k);
                                cosine_edge->real_values[pixel_counter] = edge;
                                cos_volume += edge;
                            }
                            if ( i > 0 && k > 0 ) {
                                pixel_counter                           = ReturnReal1DAddressFromPhysicalCoord(logical_x_dimension - i, j, logical_z_dimension - k);
                                cosine_edge->real_values[pixel_counter] = edge;
                                cos_volume += edge;
                            }
                            if ( i > 0 && j > 0 && k > 0 ) {
                                pixel_counter                           = ReturnReal1DAddressFromPhysicalCoord(logical_x_dimension - i, logical_y_dimension - j, logical_z_dimension - k);
                                cosine_edge->real_values[pixel_counter] = edge;
                                cos_volume += edge;
                            }
                            if ( j > 0 && k > 0 ) {
                                pixel_counter                           = ReturnReal1DAddressFromPhysicalCoord(i, logical_y_dimension - j, logical_z_dimension - k);
                                cosine_edge->real_values[pixel_counter] = edge;
                                cos_volume += edge;
                            }
                        }
...
float Image::ReturnAverageOfMinN(int number_of_pixels_to_average, float wanted_mask_radius) {
    ...
    for ( k = 0; k < logical_z_dimension; k++ ) {
        for ( j = 0; j < logical_y_dimension; j++ ) {
            for ( i = 0; i < logical_x_dimension; i++ ) {
                if ( k >= physical_address_of_box_center_z - mask_radius && k < physical_address_of_box_center_z + mask_radius && j >= physical_address_of_box_center_y - mask_radius && j < physical_address_of_box_center_y + mask_radius && i >= physical_address_of_box_center_x - mask_radius && i < physical_address_of_box_center_x + mask_radius ) {
...
void Image::CreateOrthogonalProjectionsImage(Image* image_to_create, bool include_projections, float scale_factor, float mask_radius_in_pixels) {
    ...
    if ( include_projections == true ) {
        ...
        MyDebugAssertTrue(image_to_create->logical_x_dimension == myroundint(float(logical_x_dimension) * scale_factor) * 3.0 && image_to_create->logical_y_dimension == myroundint(float(logical_y_dimension) * scale_factor) && image_to_create->is_in_real_space == true, "Output image not setup correctly");
#endif
        ...
        for ( i = 0; i < image_to_create->logical_x_dimension; i++ ) {
            if ( j < slice_one.logical_y_dimension && include_projections == true ) {
                ...
                if ( i < slice_one.logical_x_dimension ) {
                    if ( include_projections == true )
                        image_to_create->real_values[output_counter] = slice_one.ReturnRealPixelFromPhysicalCoord(i, j - slice_one.logical_y_dimension, 0);
                    else
                        image_to_create->real_values[output_counter] = slice_one.ReturnRealPixelFromPhysicalCoord(i, j, 0);
                }
                else if ( i < slice_one.logical_x_dimension * 2 ) {
                    if ( include_projections == true )
                        image_to_create->real_values[output_counter] = slice_two.ReturnRealPixelFromPhysicalCoord(i - slice_two.logical_x_dimension, j - slice_two.logical_y_dimension, 0);
                    else
                        image_to_create->real_values[output_counter] = slice_two.ReturnRealPixelFromPhysicalCoord(i - slice_two.logical_x_dimension, j, 0);
                }
                else {
                    if ( include_projections == true )
                        image_to_create->real_values[output_counter] = slice_three.ReturnRealPixelFromPhysicalCoord(i - slice_three.logical_x_dimension * 2, j - slice_three.logical_y_dimension, 0);
                    else
                        image_to_create->real_values[output_counter] = slice_three.ReturnRealPixelFromPhysicalCoord(i - slice_three.logical_x_dimension * 2, j, 0);
                }
            }
            output_counter++;
        }
...
BeamTiltScorer::BeamTiltScorer(CTF* pointer_to_wanted_ctf, Image* pointer_to_wanted_binarized_phase_diff_spectrum, float wanted_pixel_size, float wanted_mask_radius, float wanted_phase_multiplier) {
    ...
    temp_image.Allocate(pointer_to_wanted_binarized_phase_diff_spectrum->logical_x_dimension, pointer_to_wanted_binarized_phase_diff_spectrum->logical_y_dimension, true);
    beamtilt_spectrum.Allocate(pointer_to_wanted_binarized_phase_diff_spectrum->logical_x_dimension, pointer_to_wanted_binarized_phase_diff_spectrum->logical_y_dimension, true);
}
