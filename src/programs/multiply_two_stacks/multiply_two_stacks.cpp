...
IMPLEMENT_APP(MultiplyTwoStacks)

// override the DoInteractiveUserInput

void MultiplyTwoStacks::DoInteractiveUserInput( ) {

    int new_z_size = 1;

    UserInput* my_input = new UserInput("MultiplyTwoStacks", 1.0);
    ...
    std::string input_filename_two = my_input->GetFilenameFromUser("Input image file name #2", "Filename of second stack to be multiplied", "input_stack2.mrc", true);
    std::string output_filename    = my_input->GetFilenameFromUser("Output image file name", "the multiplied result", "output.mrc", false);
...
bool MultiplyTwoStacks::DoCalculation( ) {
