...
#pragma omp critical
    ...
    if ( apply_blush_denoising ) {
        ...
        const float pre_blush_pixel_size = original_pixel_size;
        float       scale_factor         = 1.0f;
        const int   original_box_size    = output_3d.density_map->logical_x_dimension;
        int         new_box_size;

        // Now handle the scaling and resampling of the output3d
        if ( std::abs(pre_blush_pixel_size - model_voxel_size) > 1e-2f )
            must_resample = true;

        if ( must_resample ) {
            scale_factor = pre_blush_pixel_size / model_voxel_size;
            new_box_size = myroundint(output_3d.density_map->logical_x_dimension * scale_factor);
            if ( new_box_size % 2 != 0 ) {
                new_box_size++;
                scale_factor = float(new_box_size) / output_3d.density_map->logical_x_dimension;
            }
            ...
            new_box_size = original_box_size;
        }
