...
#pragma omp critical
    ...
    /////////////////////////////////////////////////////////////// REMOVE BLUSH //////////////////////////////////////
    // output_3d.density_map->QuickAndDirtyWriteSlices("/data/twagner/blush_tests_cisTEM/pre_merge3d_blush.mrc", 1, output_3d.density_map->logical_z_dimension, true, original_pixel_size);
    // TODO: put in the blush logic
    // #ifdef cisTEM_USING_BLUSH
    ...
    //         // DEBUG:
    //         std::memcpy(output_3d.density_map->real_values, volume_tensor.data_ptr<float>( ), sizeof(float) * std::pow(new_box_size, 3));
    //         if ( must_resample ) {
    //             Image tmp_copy = *output_3d.density_map;
    //             tmp_copy.AddFFTWPadding( );
    //             tmp_copy.ForwardFFT( );
    //             tmp_copy.Resize(original_box_size, original_box_size, original_box_size);
    //             tmp_copy.BackwardFFT( );
    //             tmp_copy.QuickAndDirtyWriteSlices("before_masking_check.mrc", 1, tmp_copy.logical_z_dimension);
    //         }
    //         // END DEBUG
    ...
    //         float radius = outer_mask_radius * model_voxel_size + mask_edge_width / 2;
    //         radius       = std::min(radius, (new_box_size - mask_edge_width) / 2.0f + 1.0f);
    //         wxPrintf("in blush: radius == %f; outer_mask_radius == %f\n", radius, outer_mask_radius);
    //         mask_tensor = generate_radial_mask(new_box_size, radius, mask_edge_width);

    //         // TODO: like the masking protocol applied to the output_3d, fill in some of the info that is required after masking
    //         //	for (i = 0; i < output_3d.density_map->real_memory_allocated; i++) if (output_3d.density_map->real_values[i] < 0.0) output_3d.density_map->real_values[i] = -log(-output_3d.density_map->real_values[i] + 1.0);
    ...
    //             tmp_copy.Resize(original_box_size, original_box_size, original_box_size);
    //             tmp_copy.BackwardFFT( );
    //             tmp_copy.QuickAndDirtyWriteSlices("after_masking_check.mrc", 1, tmp_copy.logical_z_dimension);
    //         }
    //         // END DEBUG
    ...
    //                     torch::Tensor current_slice = mask_tensor.slice(0, z, z + model_block_size, 1).slice(1, y, y + model_block_size, 1).slice(2, x, x + model_block_size, 1).clone( );
    ...
    //         // output_3d.density_map->QuickAndDirtyWriteSlices("blush_merge3d_output.mrc", 1, output_3d.density_map->logical_z_dimension, true);
    ...
    //         // DEBUG: comment out the write back to the complete density map so that comparison between the merge3d and refine3d
    //         // blush versions with the automask is doable
    //         output_3d.density_map->WriteSlicesAndFillHeader(output_reconstruction_filtered.ToStdString( ), original_pixel_size);
    //         // DEBUG:
    //         // output_3d.density_map->QuickAndDirtyWriteSlices("post_blush_merge3d.mrc", 1, output_3d.density_map->logical_z_dimension, true, original_pixel_size);
    //         // END DEBUG
