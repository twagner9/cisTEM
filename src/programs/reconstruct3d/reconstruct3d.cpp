...
bool Reconstruct3DApp::DoCalculation( ) {
    ...
    float    resolution_limit_rec           = my_current_job.arguments[14].ReturnFloatArgument( );
    float    resolution_limit_ref           = my_current_job.arguments[15].ReturnFloatArgument( );
    float    score_weight_conversion        = my_current_job.arguments[16].ReturnFloatArgument( );
    ...
    float    padding                        = my_current_job.arguments[19].ReturnFloatArgument( );
    bool     normalize_particles            = my_current_job.arguments[20].ReturnBoolArgument( );
    bool     adjust_scores                  = my_current_job.arguments[21].ReturnBoolArgument( );
    ...
    bool     crop_images                    = my_current_job.arguments[24].ReturnBoolArgument( );
    bool     split_even_odd                 = my_current_job.arguments[25].ReturnBoolArgument( );
    bool     center_mass                    = my_current_job.arguments[26].ReturnBoolArgument( );
    ...
    // score values
    input_star_file.RemoveScoreOutliers(1.0);
    ...
    if ( resolution_limit_rec < 2.0 * pixel_size )
        resolution_limit_rec = 0.0;
    if ( resolution_limit_ref < 2.0 * pixel_size )
        resolution_limit_ref = 0.0;
    ...
    if ( resolution_limit_rec != 0.0 ) {
        ...
        if ( intermediate_box_size > original_box_size )
            intermediate_box_size = original_box_size;
    ...
    if ( ! split_even_odd )
        fsc_particle_repeat = myroundint((input_star_file.ReturnMaxPositionInStack( ) - input_star_file.ReturnMinPositionInStack( ) + 1.0) / 100.0);
    if ( fsc_particle_repeat > images_to_process )
        fsc_particle_repeat = images_to_process;
    if ( fsc_particle_repeat < 2 )
        fsc_particle_repeat = 2;
    if ( fsc_particle_repeat % 2 != 0 )
        fsc_particle_repeat++;
...
#pragma omp critical
    ...
    if ( use_input_reconstruction ) {
        ...
        if ( input_3d.mask_radius > input_3d.density_map->physical_address_of_box_center_x * pixel_size - mask_falloff )
            input_3d.mask_radius = input_3d.density_map->physical_address_of_box_center_x * pixel_size - mask_falloff;
        ...
        if ( binning_factor != 1.0 )
            input_3d.density_map->MultiplyByConstant(binning_factor);
...
#pragma omp parallel num_threads(max_threads) default(none) shared(input_3d, input_star_file, input_stack, resolution_limit_rec, original_box_size, box_size,                                                                                                                                                       \
    ...
    { // for omp
        ...
        if ( use_input_reconstruction )
            unmasked_image.Allocate(box_size, box_size, true);
        current_ctf_image.Allocate(original_box_size, original_box_size, false);
        current_beamtilt_image.Allocate(original_box_size, original_box_size, false);
...
#pragma omp critical
            ...
            //	input_parameters.SwapPsiAndPhi();
            input_particle.location_in_stack = input_parameters.position_in_stack;
            ...
            if ( use_input_reconstruction ) {
                ...
                input_particle.alignment_parameters.Init(input_parameters.phi, input_parameters.theta, input_parameters.psi, input_parameters.x_shift, input_parameters.y_shift);
            }
